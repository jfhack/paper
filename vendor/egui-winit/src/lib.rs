...
impl State {
    ...
    pub fn on_window_event(
        ...
        match event {
            ...
            WindowEvent::Ime(ime) => {
                // Upstream egui-winit 0.29 drops ALL IME events on Linux due
                // to issue #5008 (preedit text getting duplicated). That kills
                // dead-key composition: typing `'` then `a` produces neither
                // `'` nor `á`. Our local fork keeps every IME event on every
                // platform AND, when the OS *also* delivered a duplicate
                // keystroke alongside the IME commit (which is what was
                // causing the #5008 duplication in the first place), drops
                // that duplicate at commit time — see the `Ime::Commit` arm
                // below. The rest of the time `Event::Text` from
                // `KeyboardInput.text` is left alone, so plain typing without
                // a composition is unaffected.
                match ime {
                    winit::event::Ime::Enabled => {
                        // **Do NOT call `ime_event_enable()` here.** The
                        // compositor fires `Ime::Enabled` whenever a text
                        // field merely gains focus (i.e. "IME is now
                        // permitted in this window"), not when composition
                        // actually starts. egui's TextEdit, on receiving
                        // `ImeEvent::Enabled`, sets `state.ime_enabled =
                        // true`, which then *strips* `Backspace` and arrow
                        // `Event::Key` events from its event list — see
                        // egui's `remove_ime_incompatible_events`. The
                        // result is that arrows / backspace appear blocked
                        // until something flips IME off again (e.g. a
                        // composition `Commit`). We instead enter egui's
                        // IME mode lazily, only when we actually receive a
                        // non-empty `Preedit` — i.e. composition is real.
                    }
                    winit::event::Ime::Preedit(text, Some(_cursor)) => {
                        // **Skip empty preedits.** egui's TextEdit treats
                        // `ImeEvent::Preedit("")` as "clear the selection /
                        // composing region" — so any Wayland-style
                        // cursor-only IME update would silently wipe the
                        // user's currently-selected text (and arrow /
                        // backspace presses, which often emit empty preedits
                        // alongside the actual key, would appear to "do
                        // nothing" because the side-effect was a delete).
                        if !text.is_empty() {
                    ...
                    winit::event::Ime::Commit(text) => {
                        // Ignore non-text commits (control chars, empty
                        // strings) — those are typically backspace / Tab /
                        // navigation routed through IME on some Wayland
                        // setups, which we want the regular `Event::Key`
                        // path to handle, not insert as literal text.
                        let printable = !text.is_empty()
                            && text.chars().all(is_printable_char);
                        if printable {
                            // The compositor / input method on Linux often
                            // delivers the keystroke that triggered this
                            // commit through `KeyboardInput.text` as well,
                            // which we already pushed as `Event::Text`. Pop
                            // the most recent pending `Event::Text` so we
                            // don't insert the raw keystroke (`a`) right next
                            // to the composed result (`á`). On platforms
                            // that DON'T duplicate (e.g. macOS / Windows for
                            // Latin typing), this is a no-op — there won't be
                            // a recent `Event::Text` to remove because IME
                            // consumed the keystroke.
                            if let Some(idx) = self
                                .egui_input
                                .events
                                .iter()
                                .rposition(|e| matches!(e, egui::Event::Text(_)))
                            {
                                self.egui_input.events.remove(idx);
                            }
                            self.egui_input
                                .events
